...
// Add Student page component with form validation
const AddStudent = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Get student count to generate roll number
      const studentsResponse = await studentAPI.getAllStudents();
      const studentCount = studentsResponse.data.data?.length || 0;
      const newRollNumber = `STU${String(studentCount + 1).padStart(4, '0')}`;

      // Add password and rollNumber to form data
      const studentData = {
        ...formData,
      ...
      if (response.data.success) {
        alert(`Student added successfully!\n\nRoll Number: ${newRollNumber}\nDefault Password: student123\n\nStudent can register using their Roll Number or Email to set their own password.`);
        navigate('/students');
      } else {
        setErrors({ general: response.data.message || 'Failed to add student' });
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen py-8">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-gray-900 dark:text-white drop-shadow-lg">Add New Student</h1>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
              <button
                ...
                className="bg-white dark:bg-gray-700 py-2 px-6 border border-gray-300 dark:border-gray-600 rounded-lg shadow-sm text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-colors duration-200"
              >
                Cancel
              </button>
              <button
